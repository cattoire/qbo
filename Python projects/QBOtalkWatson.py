...
#        CLIENT_ACCESS_TOKEN = config["tokenAPIai"]
        WATSON_USER = config["username"]
        WATSON_PW = config["password"]
        self.WATSON_WORKSPACE = config["watsonWorkspaceID"]
        WATSON_URL=config["url"]
...
#        self.ai = apiai.ApiAI(CLIENT_ACCESS_TOKEN)
        ...
        str_resp = data["output"]["text"][0]
        self.Response = str_resp
            
        print "Response from Watson" + str_resp
    ...
    def Decode(self, audio):
	    ...
	    print "LISTEN: " + str
	    print "Going to call Watson Assistant " + self.WATSON_WORKSPACE
            response = self.assistant.message(workspace_id=self.WATSON_WORKSPACE,input={'text': str})
            print "Called the Watson Assistant"
            print response
            jsonresp = json.dumps(response)
            data=json.loads(jsonresp)
            str_resp = data["output"]["text"][0]
            print "The Watson answer is: " + str_resp
